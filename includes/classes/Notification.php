...
class Notification {
	...
	public function getNotifications($data, $limit) {
		...
		while($row = mysqli_fetch_array($query)) {
			...
			$opened = $row['opened'];
			$style = ($opened == 'no') ? "background-color: #DDEDFF;" : "background-color: #fff;";
	...
	public function insertNotification($post_id, $user_to, $type) {
		...
		switch($type) {
			...
			case 'profile_comment':
				...
				break;
			// insert firend request acceptance
			case 'accepted':
				$message = $userLoggedInName . " confirmed you as a friend";
				$link = $userLoggedIn;
			break;			
		}

		// $link = "post.php?id=" . $post_id;
		//insert friend request acceptance
		if($type !== 'accepted')
      		$link = "post.php?id=" . $post_id;
