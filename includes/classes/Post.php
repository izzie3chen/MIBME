...
class Post{
    ...
    public function submitPost($body,$user_to,$imageName) {
        ...
        $body=str_replace('\r\n',"\n",$body);//allows user to enter new line
        $body=nl2br($body);
        ...
        if ($check_empty !="" || $imageName !="") {
            ...
            foreach($body_array as $key => $value) {
                ...
                if(strpos($value, "www.youtube.com/watch?v=") === false && (strpos($value, "www") !== false || strpos($value, "http") !== false)) {
            
                    if(strpos($value, "http://") === false && strpos($value, "https://") === false) {
            
                        $value = "<p><a target='_blank' href='https://" . strip_tags($body_array[$key]) . "' title='Click to go to link' class='madeLink'>" . strip_tags($body_array[$key]) . "</a></p>";
                        $body_array[$key] = $value;
                    }
            
                            else if(strpos($value, "http://") !== false) {
            
                            $value = "<p><a target='_blank' href='" . strip_tags($body_array[$key]) . "' title='Click to go to link' class='madeLink'>" . strip_tags($body_array[$key]) . "</a></p>";
                        $body_array[$key] = $value;
                    }
            ...
            $body = mysqli_real_escape_string($this->con, $body);
// the code above makes all links clickable
    ...
    if($imagePath != "") {
        ...
        //added the code above to display youtube
        $body_array = preg_split("/\s+/", $body);
        foreach($body_array as $key => $value) {
                if(strpos($value, "www.youtube.com/watch?v=") !== false) {
                        $link = preg_split("!&!", $value);
